...
function RoomSetup({ game, setCount, update, onBack, onContinue }: { game: GameState; setCount: (n:number)=>void; update:(c:Partial<GameState>)=>void; onBack:()=>void; onContinue:()=>void }) {
  ...
  const createRoom = async () => { setOnlineError(''); const response = await fetch(`${api}/rooms`, { method:'POST', headers:{'content-type':'application/json'}, body:JSON.stringify({displayName:name,teamName}) }); const data=await response.json(); if(!response.ok) return setOnlineError(data.message ?? 'تعذر إنشاء الغرفة'); setRoomCode(data.roomCode); setPlayerId(data.playerId); setOnlineMode('lobby'); };
